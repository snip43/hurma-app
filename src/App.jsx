...
const EXECUTOR_FILTER_OPTIONS = {
  Трансфер: [
    { id: "children", label: "С детьми", terms: ["с детьми", "семьи", "семейные"] },
    { id: "child-seat", label: "Детское кресло", terms: ["детское кресло"] },
    { id: "russian-speaking", label: "Русскоговорящий", terms: ["русскоязычный", "русский язык", "русский"] },
    { id: "airport", label: "Встреча в аэропорту", terms: ["встреча в аэропорту", "аэропорт"] },
    { id: "minivan", label: "Минивэн", terms: ["минивэн"] },
    { id: "air-conditioning", label: "Кондиционер", terms: ["кондиционер"] },
    { id: "large-trunk", label: "Большой багажник", terms: ["большой багажник"] },
    { id: "intercity", label: "Межгород", terms: ["межгород", "междугородние"] },
    { id: "around-clock", label: "Круглосуточно", terms: ["круглосуточно"] },
  ],
  Клининг: [
    { id: "general-cleaning", label: "Генеральная уборка", terms: ["генеральная уборка"] },
    { id: "regular-cleaning", label: "Поддерживающая уборка", terms: ["поддерживающая уборка"] },
    { id: "after-guests", label: "После гостей", terms: ["после гостей", "уборка после гостей"] },
    { id: "after-renovation", label: "После ремонта", terms: ["после ремонта", "уборка после ремонта"] },
    { id: "windows", label: "Мытьё окон", terms: ["мытье окон", "окна"] },
    { id: "own-supplies", label: "Свои средства", terms: ["свои средства"] },
    { id: "urgent", label: "Срочный выезд", terms: ["срочный выезд"] },
    { id: "apartments", label: "Уборка апартаментов", terms: ["уборка апартаментов", "апартаменты"] },
  ],
};
...
function parseServicePhoto(url) {
  const [src = "", fragment = ""] = String(url || "").split("#", 2);
  const params = new URLSearchParams(fragment);
  const fit = params.get("fit") === "contain" ? "contain" : "cover";
  const requestedPosition = params.get("position");
  const position = ["top", "center", "bottom"].includes(requestedPosition) ? requestedPosition : "center";
  return {
    src,
    fit,
    position,
    configured: params.has("fit") || params.has("position"),
  };
}

function buildServicePhotoUrl(url, fit, position) {
  const src = parseServicePhoto(url).src;
  if (!src) return "";
  const safeFit = fit === "contain" ? "contain" : "cover";
  const safePosition = ["top", "center", "bottom"].includes(position) ? position : "center";
  return `${src}#fit=${safeFit}&position=${safePosition}`;
}

function servicePhotoStyle(url) {
  const photo = parseServicePhoto(url);
  return {
    objectFit: photo.fit,
    objectPosition: photo.fit === "contain" ? "center center" : `center ${photo.position}`,
  };
}
...
function publicStoragePath(url, bucket) {
  ...
  const cleanUrl = parseServicePhoto(url).src;
  const index = cleanUrl.indexOf(marker);
  return index === -1 ? "" : decodeURIComponent(cleanUrl.slice(index + marker.length));
}
...
function normalizeExecutorFilterText(value) {
  return String(value || "").toLocaleLowerCase("ru-RU").replaceAll("ё", "е");
}

function executorMatchesFilter(executor, option) {
  const searchable = normalizeExecutorFilterText([
    executor.name,
    executor.title,
    executor.about,
    executor.city,
  ...
  return option.terms.some((term) => searchable.includes(normalizeExecutorFilterText(term)));
}
...
async function loadSupabaseUser(authUser) {
  ...
  return {
    ...
    servicePhotoUrl: authUser.user_metadata?.service_photo_url || "",
    servicePhotoFit: authUser.user_metadata?.service_photo_fit === "contain" ? "contain" : "cover",
    servicePhotoPosition: ["top", "center", "bottom"].includes(authUser.user_metadata?.service_photo_position)
      ? authUser.user_metadata.service_photo_position
      : "center",
    subscription,
...
function toUserError(error) {
  ...
  if (/executor chat requires an accepted request/i.test(message)) {
    return "Переписка с исполнителем станет доступна после того, как он примет вашу заявку.";
  }
  ...
  return (
    <header className="topbar">
      ...
      <div className="user-tools">
        {user && !user.isGuest && user.role !== "executor" && user.searchArea ? (
    ...
    try {
      ...
      if (result && result.info) {
        ...
        if (mode === "register") {
          ...
          setSearchArea(areaOptions(defaultCity)[0]);
          setMode("login");
          window.setTimeout(() => setInfo(successInfo), 0);
        }
      }
    } catch (submitError) {
...
function ExecutorList({ service, user, onRequireSubscription, databaseExecutors, reloadExecutors }) {
  ...
  const [filterDraft, setFilterDraft] = useState({ q: "", features: [] });
  ...
  const filterOptions = EXECUTOR_FILTER_OPTIONS[service] || [];
  const items = [...databaseExecutors, ...EXECUTORS].filter((executor) => {
    if (executor.category !== service) return false;
    const textMatches = normalizeExecutorFilterText(`${executor.name} ${executor.title} ${executor.area}`)
      .includes(normalizeExecutorFilterText(q));
    const featuresMatch = selectedFeatures.every((featureId) => {
      const option = filterOptions.find((item) => item.id === featureId);
      return option ? executorMatchesFilter(executor, option) : true;
    });
    return textMatches && featuresMatch;
  });
  const activeFilterCount = (q.trim() ? 1 : 0) + selectedFeatures.length;
  const canRateExecutors = Boolean(supabaseClient && user && !user.isGuest && user.role === "client");
  const showRequestAction = Boolean(user && user.role === "client");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabaseClient
        ...
        .select("request_id, executor_id, score")
        .eq("client_id", user.id)
        .in("executor_id", executorIds),
    ]).then(([requestResult, ratingResult]) => {
        if (!active) return;
        if (requestResult.error || ratingResult.error) {
          setRatingError("Не удалось загрузить поездки и оценки. Обновите страницу и попробуйте ещё раз.");
          return;
        }
        setClientRequests(requestResult.data || []);
        setMyRatings(Object.fromEntries((ratingResult.data || []).map((rating) => [rating.request_id, Number(rating.score)])));
      });
    return () => {
      active = false;
    };
  ...
  async function saveExecutorRating(executor, request, score) {
    if (!canRateExecutors || !executor.databaseUserId || !request?.id || ratingSaving) return;
    ...
    if (error) {
      setRatingError(error.code === "23514"
        ? "Оценку можно поставить только после завершённой поездки."
        : "Не удалось сохранить оценку. Проверьте соединение и попробуйте ещё раз.");
      setRatingSaving("");
      return;
    }
    setMyRatings((ratings) => ({ ...ratings, [request.id]: score }));
    setRatingOpen("");
    try {
  ...
  function openRequestDialog(executor) {
    ...
    const firstRoute = executor.routes[0];
    setRequestDetails({
      travelAt: "",
      ...
      comment: "",
      proposedPrice: "",
      currency: normalizeCurrency(firstRoute.currency || "USD"),
    });
    setRequestDialog(executor);
  }

  function selectRequestRoute(route, index) {
    setRequestFormError("");
    ...
    setRequestDetails((details) => ({
      ...
      to: route.to,
      proposedPrice: "",
      currency: normalizeCurrency(route.currency || "USD"),
    }));
  }

  function changeRequestDetail(key, value) {
    setRequestFormError("");
    setRequestDetails((details) => ({ ...details, [key]: value }));
  }
  ...
  async function createExecutorRequest(event) {
    ...
    if (!selectedRoute) {
      setRequestFormError("Выберите поездку из списка.");
      return;
    }
    if (!requestDetails.travelAt || !requestDetails.from.trim() || !requestDetails.to.trim()) {
      setRequestFormError("Укажите дату, время, место отправления и пункт назначения.");
      return;
    }
    const travelTimestamp = new Date(requestDetails.travelAt).getTime();
    if (!Number.isFinite(travelTimestamp) || travelTimestamp < Date.now()) {
      setRequestFormError("Выберите будущую дату и время поездки.");
      return;
    }
    const proposedPrice = requestDetails.proposedPrice.trim()
      ? Number(requestDetails.proposedPrice)
      : null;
    if (proposedPrice !== null && (!Number.isFinite(proposedPrice) || proposedPrice <= 0)) {
      setRequestFormError("Предлагаемая стоимость должна быть больше нуля.");
      return;
    }
    const routePrice = formatMoney(selectedRoute.price, selectedRoute.currency || "USD");
    const travelDate = new Date(requestDetails.travelAt).toLocaleString("ru-RU", {
      dateStyle: "medium",
      timeStyle: "short",
    });
    const clientPrice = proposedPrice === null ? "" : formatMoney(proposedPrice, requestDetails.currency);
    const requestComment = [
      `Поездка: ${requestDetails.from.trim()} → ${requestDetails.to.trim()}`,
      `Дата и время: ${travelDate}`,
      `Цена исполнителя: ${routePrice}`,
      clientPrice ? `Предложение клиента: ${clientPrice}` : "",
      requestDetails.comment.trim() ? `Комментарий: ${requestDetails.comment.trim()}` : "",
    ].filter(Boolean).join("\n");
    ...
    if (error) {
      setRequestFormError(error.code === "23505"
        ? "Активная заявка этому исполнителю уже отправлена."
  ...
  const openFilters = () => {
    setFilterDraft({ q, features: [...selectedFeatures] });
    setFiltersOpen(true);
  };
  const saveFilters = (event) => {
    event.preventDefault();
    setQ(filterDraft.q);
    setSelectedFeatures(filterDraft.features);
    setFiltersOpen(false);
  };
  const toggleFeatureFilter = (featureId) => {
    setFilterDraft((draft) => ({
      ...draft,
      features: draft.features.includes(featureId)
        ? draft.features.filter((item) => item !== featureId)
    ...
    <>
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Фильтры исполнителей">
          <form className="filter-modal" onSubmit={saveFilters}>
            ...
            <div className="event-filters event-filters-dynamic">
              <input
                value={filterDraft.q}
                onChange={(event) => setFilterDraft((draft) => ({ ...draft, q: event.target.value }))}
                placeholder="Поиск исполнителя"
              />
            </div>
            <details className="executor-filter-dropdown">
              <summary>
                <span>{service === "Трансфер" ? "Параметры поездки" : "Параметры уборки"}</span>
                <strong>{filterDraft.features.length ? `Выбрано: ${filterDraft.features.length}` : "Все параметры"}</strong>
              </summary>
              <div className="executor-filter-options">
                {filterOptions.map((option) => (
                  <label className={filterDraft.features.includes(option.id) ? "selected" : ""} key={option.id}>
                    <input
                      type="checkbox"
                      checked={filterDraft.features.includes(option.id)}
                      onChange={() => toggleFeatureFilter(option.id)}
                    />
            ...
            {filterDraft.features.length ? (
              <div className="executor-filter-selected" aria-label="Выбранные параметры">
                {filterDraft.features.map((featureId) => {
                  const option = filterOptions.find((item) => item.id === featureId);
                  return option ? <span key={featureId}>{option.label}</span> : null;
                })}
              </div>
            ) : null}
            <div className="filter-modal-actions">
              <button className="secondary" type="button" onClick={() => setFilterDraft({ q: "", features: [] })}>Сброс</button>
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={`Выбор поездки у ${requestDialog.name}`}>
          <form className="filter-modal request-modal" onSubmit={createExecutorRequest}>
              ...
              {requestDialog.routes.map((route, index) => (
                <label className={`request-route-option ${selectedRouteIndex === index ? "selected" : ""}`} key={`${route.from}-${route.to}-${index}`}>
                  <input
                    ...
                    onChange={() => selectRequestRoute(route, index)}
                  />
                  <span>
            ...
            <div className="request-details">
              <label className="field request-date-field">
                <span>Дата и время поездки</span>
                <input
                  type="datetime-local"
                  min={localDateTimeValue()}
                  value={requestDetails.travelAt}
                  onChange={(event) => changeRequestDetail("travelAt", event.target.value)}
                  required
                />
              </label>
              <label className="field">
                ...
                <input
                  ...
                  onChange={(event) => changeRequestDetail("from", event.target.value)}
                  placeholder="Адрес или место отправления"
                  required
                />
              </label>
              <label className="field">
                ...
                <input
                  ...
                  onChange={(event) => changeRequestDetail("to", event.target.value)}
                  placeholder="Адрес или пункт назначения"
                  required
                />
              </label>
              <div className="request-price-field">
                <label className="field">
                  <span>Своя цена <small>(необязательно)</small></span>
                  <input
                    type="number"
                    min="1"
                    step="0.01"
                    inputMode="decimal"
                    value={requestDetails.proposedPrice}
                    onChange={(event) => changeRequestDetail("proposedPrice", event.target.value)}
                    placeholder="Например, 12"
                  />
                </label>
                <label className="field request-currency-field">
                  <span>Валюта</span>
                  <select value={requestDetails.currency} onChange={(event) => changeRequestDetail("currency", event.target.value)}>
                    {CURRENCIES.map((currency) => <option key={currency.code} value={currency.code}>{currency.label}</option>)}
                  </select>
                </label>
              </div>
              <label className="field request-comment-field">
                <span>Комментарий исполнителю</span>
                <textarea
                  value={requestDetails.comment}
                  onChange={(event) => changeRequestDetail("comment", event.target.value)}
                  placeholder="Например, номер рейса, количество пассажиров или детское кресло"
                  rows="3"
                />
              </label>
            </div>
            {requestFormError ? <div className="error-state request-form-error">{requestFormError}</div> : null}
            <div className="filter-modal-actions">
      ...
      <div className="cards-grid">
        {items.map((executor) => {
          ...
          const executorRequests = executor.databaseUserId
          ...
          return (
            <article className="executor-card" key={executor.id}>
              <div className="executor-card-body">
                ...
                {canRateExecutors && executor.databaseUserId && ratingRequest ? (
                  <div className="executor-rating-control">
                    <button className="rating-toggle" type="button" onClick={() => setRatingOpen((current) => current === executor.id ? "" : executor.id)}>
                      ...
                      {ownRating ? `Ваша оценка: ${ownRating}` : ratingButtonLabel}
                    </button>
                    {ratingEditorOpen ? (
                      <div className="rating-stars" role="group" aria-label={`${ratingButtonLabel} с ${executor.name}`}>
                        {[1, 2, 3, 4, 5].map((score) => (
                          <button
                            ...
                            onClick={() => saveExecutorRating(executor, ratingRequest, score)}
...
function Messages({ chatId, setChatId, user, onServices, onChat, onProfile, externalConversationFallbacks = {} }) {
  ...
  async function loadContacts() {
    ...
    if (profilesResult.error || requestsResult.error) {
      setChatError(toUserError(profilesResult.error || requestsResult.error));
      return;
    }
    const eligibleOppositeRoleIds = new Set((requestsResult.data || []).map((request) => (
      isClient ? request.executor_id : request.client_id
    )).filter(Boolean));
    const visibleProfiles = (profilesResult.data || []).filter((profile) => {
      if (isClient && profile.role === "executor") {
        return eligibleOppositeRoleIds.has(profile.id);
      }
      if (isExecutor && profile.role === "client") {
        return eligibleOppositeRoleIds.has(profile.id);
      }
      return true;
    });
    setContacts(visibleProfiles.map(normalizeContact));
  }
          ...
          {contactPicker ? (
            <div className="modal-backdrop contact-modal-backdrop" role="dialog" aria-modal="true" aria-label="Добавить собеседника">
              ...
              <div className="wa-notice">
                {user.role === "client"
                  ? "Исполнитель появится здесь после того, как примет вашу заявку. С другими клиентами можно начать обычную переписку."
                  : "Клиент появится здесь после того, как вы примете его заявку. С другими исполнителями можно начать обычную переписку."}
              </div>
              ...
              <div className="contact-list">
                ...
                {!visibleContacts.length ? <div className="wa-notice">Подходящих контактов пока нет.</div> : null}
              </div>
              </div>
            </div>
          ) : null}
          {!user.isGuest && !contactPicker ? <button className="wa-floating-add" type="button" onClick={openContactPicker} aria-label="Добавить собеседника"></button> : null}
        </div>
      </div>
    );
  }
...
function RoutePriceEditor({ route, index, onChange, exchangeRates }) {
  ...
  return (
    <label className="route-price-field">
      <span className="route-price-label">
        <span>Цена</span>
        {converted ? <small className="route-conversion">{converted}</small> : null}
      </span>
...
function ServicePhotoCropper({ file, onCancel, onConfirm, onError }) {
  ...
  const cropMetrics = useMemo(() => {
    ...
    return {
      ...
      displayWidth,
      displayHeight,
      maxX: Math.max(0, (displayWidth - frameSize.width) / 2),
      maxY: Math.max(0, (displayHeight - frameSize.height) / 2),
    };
  }, [frameSize.height, frameSize.width, naturalSize.height, naturalSize.width, zoom]);

  function clampOffset(value) {
    return {
      x: Math.max(-cropMetrics.maxX, Math.min(cropMetrics.maxX, value.x)),
      y: Math.max(-cropMetrics.maxY, Math.min(cropMetrics.maxY, value.y)),
    };
  }
  ...
  async function useCrop() {
    ...
    setCropping(true);
    try {
      const sourceWidth = frameSize.width / cropMetrics.scale;
      const sourceHeight = frameSize.height / cropMetrics.scale;
      const sourceX = Math.max(
        0,
        Math.min(naturalSize.width - sourceWidth, (naturalSize.width - sourceWidth) / 2 - offset.x / cropMetrics.scale)
      );
      const sourceY = Math.max(
        0,
        Math.min(naturalSize.height - sourceHeight, (naturalSize.height - sourceHeight) / 2 - offset.y / cropMetrics.scale)
      );
      ...
      onConfirm(new File([blob], `${baseName}-cropped.jpg`, {
        type: "image/jpeg",
  ...
  return (
    <div className="modal-backdrop photo-crop-backdrop" role="dialog" aria-modal="true" aria-label="Выбор фрагмента фотографии">
      <section className="photo-crop-dialog">
        <div className="photo-crop-heading">
          <div>
            <h2>Выберите фрагмент</h2>
            <p>Перетащите фотографию и оставьте в рамке нужный фрагмент.</p>
          </div>
          <button className="photo-crop-close" type="button" onClick={onCancel} aria-label="Закрыть">×</button>
        </div>
        <div
          ref={frameRef}
          className={`photo-crop-frame ${dragging ? "dragging" : ""}`}
        ...
        >
          {sourceUrl ? (
            <img
              ref={imageRef}
              src={sourceUrl}
              alt="Фотография для кадрирования"
              draggable="false"
              onLoad={(event) => {
                setNaturalSize({
              ...
              style={{
                width: cropMetrics.displayWidth || undefined,
                height: cropMetrics.displayHeight || undefined,
        ...
        <div className="photo-crop-actions">
          <button className="ghost" type="button" onClick={onCancel} disabled={cropping}>Отмена</button>
          <button className="primary" type="button" onClick={useCrop} disabled={cropping || !naturalSize.width}>
            {cropping ? "Подготавливаем..." : "Использовать фото"}
          </button>
        </div>
      </section>
    </div>
...
function Profile({ user, setUser, reloadExecutors, onBack, mode = "profile" }) {
  ...
  const [servicePhotoCropFile, setServicePhotoCropFile] = useState(null);
  const [servicePhotoFit, setServicePhotoFit] = useState(user.servicePhotoFit || initialServicePhoto.fit);
  const [servicePhotoPosition, setServicePhotoPosition] = useState(user.servicePhotoPosition || initialServicePhoto.position);
  ...
  const servicePhotoPreview = useMemo(
    () => servicePhotoFile ? URL.createObjectURL(servicePhotoFile) : parseServicePhoto(executorDraft.photoUrl || draft.servicePhotoUrl).src,
    [servicePhotoFile, executorDraft.photoUrl, draft.servicePhotoUrl]
  );
  ...
  const availableFeatures = EXECUTOR_FEATURES[executorCategory] || [];
    ...
    supabaseClient
      ...
      .then(({ data, error }) => {
        ...
        if (error) {
          ...
          const photoUrl = data.photo_url || user.servicePhotoUrl || "";
          const photoSettings = parseServicePhoto(photoUrl);
          setExecutorDraft({
          ...
          setServicePhotoPosition(photoSettings.configured ? photoSettings.position : (user.servicePhotoPosition || photoSettings.position));
        } else {
          const photoSettings = parseServicePhoto(user.servicePhotoUrl || "");
          setExecutorDraft((current) => ({ ...current, photoUrl: user.servicePhotoUrl || current.photoUrl }));
          setServicePhotoFit(user.servicePhotoFit || photoSettings.fit);
          setServicePhotoPosition(user.servicePhotoPosition || photoSettings.position);
        }
  ...
  }, [user.id, user.role, user.servicePhotoFit, user.servicePhotoPosition, user.servicePhotoUrl]);
  ...
  async function save(event) {
    ...
    let servicePhotoUploaded = null;
    try {
      if (avatarFile) avatarUploaded = await uploadPublicImage(PROFILE_AVATAR_BUCKET, user.id, avatarFile, "avatar");
      if (user.role === "executor" && servicePhotoFile) {
        servicePhotoUploaded = await uploadPublicImage(SERVICE_IMAGE_BUCKET, user.id, servicePhotoFile, "service");
      }
      const nextAvatarUrl = avatarUploaded?.url || draft.avatarUrl || "";
      const currentServicePhotoUrl = executorDraft.photoUrl || draft.servicePhotoUrl || "";
      const nextServicePhotoUrl = user.role === "executor"
        ? buildServicePhotoUrl(servicePhotoUploaded?.url || currentServicePhotoUrl, servicePhotoFit, servicePhotoPosition)
        : "";
      ...
      if (user.role === "executor") {
        const { error: authError } = await supabaseClient.auth.updateUser({
          data: {
            service_photo_url: nextServicePhotoUrl,
            service_photo_fit: servicePhotoFit,
            service_photo_position: servicePhotoPosition,
          },
        });
      ...
      if (avatarUploaded && draft.avatarUrl && draft.avatarUrl !== nextAvatarUrl) {
        await removePublicImage(PROFILE_AVATAR_BUCKET, draft.avatarUrl);
      }
      if (servicePhotoUploaded && currentServicePhotoUrl && parseServicePhoto(currentServicePhotoUrl).src !== parseServicePhoto(nextServicePhotoUrl).src) {
        await removePublicImage(SERVICE_IMAGE_BUCKET, currentServicePhotoUrl);
      }
      const nextUser = {
        ...draft,
        avatarUrl: nextAvatarUrl,
        servicePhotoUrl: nextServicePhotoUrl,
        servicePhotoFit,
        servicePhotoPosition,
      };
      setDraft(nextUser);
      setUser(nextUser);
      setAvatarFile(null);
      setServicePhotoFile(null);
      setExecutorDraft((current) => ({ ...current, photoUrl: nextServicePhotoUrl }));
      setLocked(true);
      if (user.role === "executor") await reloadExecutors();
    } catch (error) {
      if (avatarUploaded?.url) await removePublicImage(PROFILE_AVATAR_BUCKET, avatarUploaded.url);
      if (servicePhotoUploaded?.url) await removePublicImage(SERVICE_IMAGE_BUCKET, servicePhotoUploaded.url);
  ...
  function chooseServicePhoto(event) {
    ...
    setServicePhotoCropFile(file);
    event.target.value = "";
  }

  function confirmServicePhoto(file) {
    setServicePhotoFile(file);
    setServicePhotoCropFile(null);
    setServicePhotoFit("cover");
    setServicePhotoPosition("center");
    setLocked(false);
    setProfileError("");
  }
  ...
  async function publishExecutor(event) {
    ...
    if (!executorDraft.photoUrl) {
      setExecutorError("Добавьте фотографию автомобиля или услуги в настройках профиля. Без неё публикация недоступна.");
      return;
    }
    ...
    try {
      const nextPhotoUrl = executorDraft.photoUrl;
      const routePrices = routes.map((route) => route.price);
      const payload = {
        user_id: user.id,
        display_name: draft.name.trim(),
        category: executorCategory === "Клининг" ? "cleaning" : "transfer",
    ...
    <div className="profile-stack">
      {mode === "profile" ? <div className="panel profile-panel">
        ...
        <form className="form" onSubmit={save}>
          {user.role === "executor" ? (
            <div className="profile-avatar-editor executor-profile-overview">
              ...
              <div className="executor-profile-identity">
          ...
          {user.role === "executor" ? (
            <div className="service-photo-editor profile-service-photo-editor">
              <div className="service-photo-heading">
                <span className="service-photo-label">{executorCategory === "Трансфер" ? "Фото автомобиля" : "Фото услуги"}</span>
                <small>Эта фотография будет показана клиентам в карточке объявления.</small>
              </div>
              <div className="service-photo-preview">
                {servicePhotoPreview ? (
                  <img
                    src={servicePhotoPreview}
                    alt={executorCategory === "Трансфер" ? "Фото автомобиля" : "Фото услуги"}
                  />
                ) : (
                  <span>{executorCategory === "Трансфер" ? "Добавьте фотографию автомобиля" : "Добавьте фотографию своей услуги"}</span>
                )}
              </div>
              <label className="secondary media-picker">
              ...
              <small>После выбора можно передвинуть и увеличить фотографию. JPG, PNG или WebP · до 8 МБ</small>
              {servicePhotoCropFile ? (
                <ServicePhotoCropper
                  file={servicePhotoCropFile}
                  onCancel={() => setServicePhotoCropFile(null)}
                  onConfirm={confirmServicePhoto}
                  onError={(error) => setProfileError(toUserError(error))}
                />
              ) : null}
            </div>
          ) : null}
        ...
        <div className="panel profile-panel executor-publish-panel">
          ...
          {executorLoading ? <div className="form-info">Загружаем объявление...</div> : (
            <form className="form" onSubmit={publishExecutor}>
              {executorCategory === "Клининг" ? <label className="field"><span>Район работы</span><select value={executorDraft.serviceArea} onChange={(event) => changeExecutor("serviceArea", event.target.value)}>{areas.map((area) => <option key={area}>{area}</option>)}</select></label> : null}
