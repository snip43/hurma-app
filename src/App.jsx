...
function Profile({ user, setUser, reloadExecutors, onBack, mode = "profile" }) {
  ...
  function chooseServicePhoto(event) {
    ...
    setServicePhotoFit("contain");
    setServicePhotoPosition("center");
    setLocked(false);
    event.target.value = "";
  }

  function changeServicePhotoFit(value) {
    setServicePhotoFit(value);
    setLocked(false);
    setProfileError("");
  }

  function changeServicePhotoPosition(value) {
    setServicePhotoPosition(value);
    setLocked(false);
    setProfileError("");
  }
        ...
        <form className="form" onSubmit={save}>
          ...
          {user.role === "executor" ? (
            <div className="service-photo-editor profile-service-photo-editor">
              ...
              <div className={`service-photo-preview ${servicePhotoFit === "contain" ? "contain-mode" : ""}`}>
                {servicePhotoPreview ? (
                  <img
                    src={servicePhotoPreview}
                    style={{
                      objectFit: servicePhotoFit,
                      objectPosition: servicePhotoFit === "contain" ? "center center" : `center ${servicePhotoPosition}`,
                    }}
              ...
              {servicePhotoPreview ? (
                <div className="service-photo-display-settings">
                  <span>Как показать фотографию</span>
                  <div className="service-photo-fit-buttons" role="group" aria-label="Режим отображения фотографии">
                    <button className={servicePhotoFit === "contain" ? "active" : ""} type="button" onClick={() => changeServicePhotoFit("contain")}>
                      Показать целиком
                    </button>
                    <button className={servicePhotoFit === "cover" ? "active" : ""} type="button" onClick={() => changeServicePhotoFit("cover")}>
                      Заполнить кадр
                    </button>
                  </div>
                  {servicePhotoFit === "cover" ? (
                    <label className="service-photo-position">
                      <span>Какой фрагмент оставить в кадре</span>
                      <select value={servicePhotoPosition} onChange={(event) => changeServicePhotoPosition(event.target.value)}>
                        <option value="top">Верх фотографии</option>
                        <option value="center">Центр фотографии</option>
                        <option value="bottom">Низ фотографии</option>
                      </select>
                    </label>
                  ) : (
                    <small>Вся фотография поместится в окно без обрезки.</small>
                  )}
                </div>
              ) : null}
